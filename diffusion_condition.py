...
class MaskDiffusion(DiscreteDiffusionMatrixBase):
    def __init__(self,
        ...
        self.mask = torch.nn.functional.one_hot(torch.tensor(self.tokenizer.mask_token_id), num_classes=self.dim).unsqueeze(1).repeat(1, self.dim).float()
        self.device = device
        self.state = self._create_state()
        self.word_freq = word_freq.to(device)
...
def compute_kl_reverse_process(x_start,
    ...
    if predict_x0 and hybrid_lambda > 0.0:
        ...
        hybrid_loss = torch.tensor([0.0], device=p_t.device)

    hybrid_loss = (hybrid_loss * target_mask).sum()

    base_loss = (losses.cross_entropy_with_probs(probs=p_t, targets=x_start) * target_mask).sum() \
        if t == 0 else (losses.kl_divergence_with_probs(q_t, p_t) * target_mask).sum()
...
def discrete_diffusion_elbo(
    ...
    init_state = State(
        t=torch.tensor([diffusion.num_steps - eval_step_size], device=x_start.device),
        log_likelihood=torch.tensor(0.0, device=x_start.device),
    )
    ...
    if length_probs is not None:
        length_probs = torch.tensor(length_probs, device=x_start.device)
...
def discrete_diffusion_predict_fn(
  ...
  """
    ...
    x = torch.where(target_mask.bool(), mask_token_id, input_ids)

    if predict_x0:
        init_state = SamplingState(x, x, torch.tensor([num_steps], device=input_ids.device))
    else:
        init_state = SamplingState(x, None, torch.tensor([num_steps], device=input_ids.device))
